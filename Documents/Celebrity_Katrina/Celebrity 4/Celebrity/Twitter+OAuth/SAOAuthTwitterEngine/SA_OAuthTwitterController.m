...
+ (SA_OAuthTwitterController *) controllerToEnterCredentialsWithTwitterEngine: (SA_OAuthTwitterEngine *) engine delegate: (id <SA_OAuthTwitterControllerDelegate>) delegate {
    ...
    return [SA_OAuthTwitterController controllerToEnterCredentialsWithTwitterEngine: engine delegate: delegate forOrientation:UIInterfaceOrientationPortrait|UIInterfaceOrientationPortraitUpsideDown];
}
...
- (id) initWithEngine: (SA_OAuthTwitterEngine *) engine andOrientation:(UIInterfaceOrientation)theOrientation {
	if (self = [super init]) {
        ...
        CGRect webFrame = CGRectMake(0, 44, [[UIScreen mainScreen] bounds].size.width, 
...
//	} else {
		...
		_backgroundView.frame =  CGRectMake(0, 44, [[UIScreen mainScreen] bounds].size.width, 
                                            [[UIScreen mainScreen] bounds].size.height-44);
		_navBar = [[[UINavigationBar alloc] initWithFrame: CGRectMake(0, 0, [[UIScreen mainScreen] bounds].size.width, 44)] autorelease];
//	}
    if (UI_USER_INTERFACE_IDIOM() == UIUserInterfaceIdiomPad) {
        
        _navBar.tintColor = [UIColor colorWithHue:0.6 saturation:0.33 brightness:0.69 alpha:0];
    }
...
- (UIToolbar *) pinCopyPromptBar {
    ...
    if (_pinCopyPromptBar == nil){
		...
		//_pinCopyPromptBar.barStyle = UIBarStyleDefault;
